...
letDefintion
        ...
        | ('var'|'hvar') ID /* todo variableRange?*/ '=' recordExpression ';'

       | ('var'|'hvar') ID ('[' expression ']')+ /* todo variableRange?*/ ('=' (recordExpression|'*') )? ';' //multi-dimensional array is supported
...
recordElement
     ...
     : (expression ('(' e2=expression ')')?) => e1=expression ('(' e2=expression ')')? //e2 means the number of e1, by default it's 1
...
paralDef
            ...
            : ID ':' '{' additiveExpression ((',' additiveExpression)* | '..' additiveExpression)  '}'
...
channelExpr
            ...
            : ID ('[' additiveExpression']')?  channelExpr_refactor

            | eventExpr

            ;
channelExpr_refactor
	:	'!' expression ('.' expression)*  '->' channelExpr
	|	 '?' ('[' conditionalOrExpression ']')? expression ('.' expression)*  '->' channelExpr //here expression is either a single variable or expression that has no global variables. Optional conditionalOrExpression is the guard condition that stop the channel input event if the condition is false. 
	;
...
ifExpr  :  // no such thing, atomicIfExpr  
